...
impl<const CAP: usize> StateChangeCache<CAP> {
    ...
    pub fn unwind_state_changes(&mut self, block_to_unwind: u64) -> Result<Vec<AMM>, String> {
        let cache = &mut self.cache;

        if block_to_unwind < self.oldest_block {
            return Err(format!(
                "Block to unwind {} < oldest block in cache {}",
                block_to_unwind, self.oldest_block
            ));
        }
        ...
        Ok(self.flatten_state_changes(state_changes))
    }
