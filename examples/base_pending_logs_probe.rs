...
async fn main() -> eyre::Result<()> {
    ...
    let Some(reply) = subscribe_reply else {
        return Err(eyre::eyre!("pendingLogs subscribe stream ended immediately"));
    };
    let reply = reply?;
    ...
    loop {
        ...
        if sampled < sample_limit {
            ...
            println!(
                "[sample #{sampled}] result_keys={} address={} topic_count={} block={} tx={}",
                joined_keys(&result_keys),
                result.get("address").and_then(Value::as_str).unwrap_or("<none>"),
                result
                    .get("topics")
